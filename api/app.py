...
# This allows the app to run as a Gradio app on Hugging Face, but still expose the FastAPI /predict endpoint!
import gradio as gr

# Create a simple dummy UI for the Hugging Face space
def ui_test(img):
    return "API is running in the background. Send POST requests to /predict!"
...
demo = gr.Interface(
    ...
    outputs="text",
    title="OmniDrive API",
    description="This space hosts the backend API for OmniDrive. The /predict POST endpoint is active."
)
